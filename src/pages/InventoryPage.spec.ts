...
class InventoryPage {
  ...
  public async getOrAddItemsInList(ctx: TestController, addItems: boolean, addSpecificItem?: string): Promise<Item[]>{
    ...
    for (let i = 0; i < inventorySize; i++) {
      const btnAddItem = this._inventoryItems.child(i).find('button.btn.btn_primary.btn_small.btn_inventory');
      const itemPrice = await getInnerTextNumberFromItem(this._inventoryItems.child(i),'div.inventory_item_price');
      const itemName = await this._inventoryItems.child(i).find('div.inventory_item_name').innerText;

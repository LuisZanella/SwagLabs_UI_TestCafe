...
class CartPage {
  ...
  constructor() {
    this._itemsInCart = Selector("#cart_contents_container").find(
      "div.cart_item"
    );
  ...
  public async getCartItems(ctx: TestController): Promise<Item[]> {
    ...
    for (let i = 0; i < inventorySize; i++) {
      ...
      const currentItemName = await item.find("div.inventory_item_name")
        .innerText;
      const currentQuantity = await getInnerTextNumberFromItem(
        item,
        "div.cart_quantity"
      );
      const currentPrice = await getInnerTextNumberFromItem(
        item,
